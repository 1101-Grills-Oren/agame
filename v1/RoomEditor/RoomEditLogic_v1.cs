...
class RoomEditLogic_v00000001
{
    ...
    {
        ParticleID.count+=1;
        IDMaps.ParticleNames=AppendArrayValue(IDMaps.ParticleNames,name);
        ParticleID.Width=AppendArrayValue(ParticleID.Width,1);
        ParticleID.Height=AppendArrayValue(ParticleID.Height,1);
        ParticleID.NumVariants=AppendArrayValue(ParticleID.NumVariants,1);
        ParticleID.Behavior=AppendArrayValue(ParticleID.Behavior,0);
        ParticleID.Textures=AppendArrayValue(ParticleID.Textures,LoadTexture("particle_"+name));
    }
    public static void RemoveParticleFromList(int id)
    {
        ParticleID.count-=1;
        IDMaps.ParticleNames=PopArrayValue(IDMaps.ParticleNames,id);
        ParticleID.Width=PopArrayValue(ParticleID.Width,id);
        ParticleID.Height=PopArrayValue(ParticleID.Height,id);
        ParticleID.NumVariants=PopArrayValue(ParticleID.NumVariants,id);
        ParticleID.Behavior=PopArrayValue(ParticleID.Behavior,id);
        ParticleID.Textures[id].Dispose();
        ParticleID.Textures=PopArrayValue(ParticleID.Textures,id);
        foreach(Particle p in Game1.instance.particles)
        {
            if (p.id == id)
            {
                p.dead=true;
    ...
    public static string ParticleToInfo(int id)
    {
        return IDMaps.ParticleNames[id]+"="+id+"("+ParticleID.Width[id]+"."+ParticleID.Height[id]+"."+ParticleID.NumVariants[id]+"."+ParticleID.Behavior[id]+");";
    }
    ...
    public static ParticleData InfoToParticle(string info)
    {
        string name=info.Split("=")[0];
        info=info.Split("=")[1];
        string id=info.Split("(")[0];
        ...
        string width=info.Split(".")[0];
        string height=info.Split(".")[1];
        string numVariants=info.Split(".")[2];
        string behavior=info.Split(".")[3];
        
        return new ParticleData{name=name,id=int.Parse(id),width=int.Parse(width),height=int.Parse(height),numVariants=int.Parse(numVariants),behavior=int.Parse(behavior)};
    }
    ...
    public static string GetParticleInfos()
    {
        String v="";
    ...
    public static void LoadRoomDatas(string datas)
    {
        ...
        n=datas.Split("-----")[2].Trim();
        string[] particledatas=n.Substring(0,n.Length-1).Split(";");
        ParticleID.count=particledatas.Length;
        ParticleID.Behavior=new int[ParticleID.count];
        ParticleID.Width=new int[ParticleID.count];
        ParticleID.Height=new int[ParticleID.count];
        ParticleID.NumVariants=new int[ParticleID.count];
        IDMaps.ParticleNames=new string[particledatas.Length];
        foreach(string particledata in particledatas)
        {
            ParticleData d=InfoToParticle(particledata.Trim());
            IDMaps.ParticleNames[d.id]=d.name;
            ParticleID.Width[d.id]=d.width;
            ParticleID.Height[d.id]=d.height;
            ParticleID.Behavior[d.id]=d.behavior;
            ParticleID.NumVariants[d.id]=d.numVariants;
            ParticleID.Textures[d.id]=LoadTexture("particle_"+d.name);
        }
    }
